...
import com.projectm.project.domain.ProjectMenuApply;
...
public class AuthorityControoler extends BaseController {
    ...
    @ResponseBody
    public AjaxResult menuDel(ProjectMenuApply menuEdit) {
        Integer id = menuEdit.getId();

        ProjectMenu projectMenu = projectMenuService.getProjectMenuById(id);
        if(menuEdit.is_inner()) {
            projectMenu.setIsinner(1);
        }else {
            projectMenu.setIsinner(0);
        }
        return AjaxResult.success("修改成功", projectMenuService.updateById(projectMenu));
    }
