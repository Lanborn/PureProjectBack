...
public class MemberController  extends BaseController {
    ...
    @PostMapping("/project_member/_joinByInviteLink")
    @ResponseBody
    public AjaxResult inviteMember_Link(@RequestParam Map<String, Object>mmap){
        Map loginMember = getLoginMember();
        ...
        if(MapUtils.isEmpty(inviteLink) || today.compareTo((String) inviteLink.get("over_time")) != 1) {
            return AjaxResult.error("链接已过期");
        }
        if(inviteType.equals("project")){
            ProjectMember projectMember = projectMemberService.inviteMember(memberCode, sourceCode, 0);
//            Map currentProject = projectService.getProjectByCode(sourceCode);
        }
        Organization currentOrganization = new Organization();
        Map memberAccount = memberAccountService.getMemberAccountByMemCode(memberCode);
        IPage<Map> ipage = Constant.createPage(mmap);
        IPage<Map> allOrganization = organizationService.getAllOrganizationByMemberCode(ipage,memberCode);
//        allOrganization.
